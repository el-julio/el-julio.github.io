let desc_proj = [
	{
		"image": "affiche_crypto",
		"nom": "2024 - Une affiche pour un site historique",
		"description": "L'un de mes travaux les plus récent. Dans le cadre de mes études, j'ai eu comme projet de réaliser une affiche promotionnelle pour le site historique des cryptoportiques en Arles. Un exercice atypique puisqu'il fallait ici moderniser l'image d'un site historique pour le rendre attractif. J'ai fait le choix d'axer mon travail sur la composition, quelque chose de géométrique et moderne permettant un agencement équilibré et cohérent des informations. Et j'y ai évidemment intégré ma couleur de prédilection, l'orange.",
		"format": "a4",
    },
	{
		"image": "affiche_dad",
		"nom": "2023 - Une affiche pour mon père",
		"description": "Avant l'ouverture de sa boutique dont j'ai réalisé le logo, mon père a ouvert une petite galerie temporaire et m'a demandé de faire une affiche. Ce qui m'avais le plus marqué, c'était la couleur qu'il avait apporté à un vieux local en vieille pierre, de par tous les objets, toutes les œuvres qu'il a pu y exposer. J'y suis allé, j'ai pris plusieurs photos et j'ai finis par choisir une des peintures exposée pour le fond de l'affiche. Je voulais appuyer un côté fait main, alors j'ai fait la typo à main levé sur ma tablette graphique avec un brush spécial, puis je l'ai retouché pour 'l'abimer'.",
		"format": "a4",
    },
	{
		"image": "cover_liquid",
		"nom": "2020 - Cover liquide",
		"description": "En 2020 quand j'ai sérieusement commencé mon apprentissage autodidacte, je suis entré en contact avec un ami musicien de mon frère, Q1kill. J'avais rapidement travaillé sur un petit exemple de pochette, cet exemple lui a plu. J'ai par la suite réalisé de multiples pochettes pour lui. Certaines ont servi et d'autres non, mais j'ai utilisé ce sujet pour m'entraîner et tenter de nouvelles choses. Cette première pochette a été pour moi l'occasion d'essayer un effet de 3D sur Photoshop, en utilisant un dégradé comme base avec un pinceau, on obtient cette effet 'liquide' assez flottant.",
		"format": "square",
    },
	{
		"image": "cover_montagne",
		"nom": "2020 - Cover montagne",
		"description": "Cette pochette fait partie de celles qui n'ont pas servis à l'artiste mais m'ont servi pour ma démarche d'apprentissage. Une occasion pour moi de m'essayer à d'autres formes de composition, à des effets de retouche sur les photos (que je trouve, avec plusieurs années de recul, bien trop prononcés) et à des jeux de contrastes avec les couleurs de la deuxième photo. 4 ans après sa réalisation, elle fait toujours partie de mes travaux préférés. Je trouve cette pochette surprenamment avancée par rapport au niveau que je pouvais avoir 4 ans en arrière.",
		"format": "square",
    },
	{
		"image": "cover_neon",
		"nom": "2020 - Cover néon",
		"description": "Cette cover avait été plutôt appréciée par Q1kill. Pour être honnête, je l'ai modifié. Il y avait à la base une immense typo blanche en bien milieu qui venait complètement perturber l'ensemble, une petite maladresse du moi d'il y a 4 ans. Cette pochette est le résultat d'un des nombreux tutoriels que j'ai suivi pour apprendre et de mes modifications et ajouts personnels.",
		"format": "square",
    },
	{
		"image": "cover_nuit",
		"nom": "2020 - Cover nocturne",
		"description": "Cette pochette fait partie de celles qui ont servi. C'est le résultat d'un des nombreux tutoriels que j'ai suivi, un tutoriel de l'excellente chaîne YouTube Geoffrey – Apprendre le Graphisme. C'était la première fois que j'utilisait des masques d'écrêtage, une fonction de Photoshop qui permet de placer une image uniquement sur la surface d'une autre.",
		"format": "square",
    },
	{
		"image": "space_portal",
		"nom": "2022 - Space Portal",
		"description": "J'ai très tôt été passionné par l'espace et la science-fiction grâce aux livres, aux films et aux documentaires. Certains de mes projets découlent logiquement de ces passions. Pour ce projet j'ai souhaité essayer le glass morphism, un courant dans le graphisme qui utilise des textures transparentes comme du verre. Je m'en suis servi pour faire quelque chose de futuriste qui correspond bien à ce que je voulais représenter : une sorte de carte, de portail permettant le voyage interstellaire. On le voit dans la composition avec ce grand cercle placé au centre et ces formes abstraites du fond qui forment presque une spirale s'y dirigeant.",
		"format": "square",
    },
	{
		"image": "solar_eyelander",
		"nom": "2021 - Solar Eyelander",
		"description": "Un jour, par hasard, pendant un long moment d'ennui (un cours de math) j'ai dessiné un petit logo dans le coin de mon cahier, le mot Solar cerclé d'un anneau. En rentrant, je l'ai vectorisé sur Illustrator et j'ai commencé à travailler. Découlant de ce premier logo, un premier t-shirt, très simple, puis celui-ci. Je suis passionné par l'espace, et depuis tout petit par la planète Mars, je lui ai donc dédié ce t-shirt. C'est également le premier de mes projets que j'ai porté sur un support physique. Sur la plateforme Everpress, j'ai créé une campagne de vente, et une dizaine de personnes ont acquis ce t-shirt. J'ai toujours le mien, il est très abîmé, mais je le porte toujours de temps à autre.",
		"format": "square",
    },
	{
		"image": "solar_void",
		"nom": "2023 - Solar Void",
		"description": "Longtemps après la création du t-shirt Eyelander, je me suis penché sur la réalisation d'un nouveau t-shirt. Après en avoir dédié un à Mars, j'ai dédié celui-ci à des concepts futuristes comme le voyage interstellaire ou le transhumanisme. Tous deux symbolisé par la 'collone vertébrale le long du dos et le portail en son centre, inspiré de ce que j'avais déjà fait en 2022 avec Space Portal, mais en remaniant le style",
		"format": "square",
    },
	{
		"image": "cover_skull",
		"nom": "2020 - Cover skull",
		"description": "Une pochette que j'ai réalisé pour m'entraîner. Celle-ci m'a permis en particulier de découvrir sur Photoshop les effets de style, les courbes de transfert dégradé et les réglages courbe, qui permettent de donner cette saturation aux couleurs. J'ai aussi commencé à installer de nouvelles polices d'écriture pour coller à l'ambiance de chaque nouveau design. Également l'outil fluidité, qui permet en quelque sorte de mixer un élément pour lui donner une nouvelle forme, comme vous pouvez le voir derrière le crâne. Mes fonds sont souvent unis, mais je commence à ajouter des éléments de décoration, comme la règle de mesure sur le côté droit.",
		"format": "square",
    },
	{
		"image": "poster_pondy",
		"nom": "2023 - Un poster pour le Pondichery",
		"description": "L'histoire derrière ce poster est assez comique. J'ai fait tous mes jobs d'été au restaurant indien Le Pondichéry, à Sommières. Un jour, j'ai mal nettoyé un cuiseur à riz, et du riz a commencé à brûler à l'intérieur, le rendant inutilisable. On peut dire que j'ai un peu fait ce poster pour me faire pardonner. J'ai mixé la devanture du restaurant que j'ai simplifié et restylé et le Taj-Mahal. Il a beaucoup plu à ma patronne qui lui a immédiatement trouvé une place dans le restaurant.",
		"format": "a4",
    },
	{
		"image": "illu_dune",
		"nom": "2021 - Mes premières illustrations",
		"description": "Quand j'ai commencé à travailler sur Illustrator, après avoir passé des heures à appréhender le logiciel, j'ai voulu créer une petite affiche. J'aimais beaucoup les poster en 2d représentant des paysages que je voyais sur les réseaux. J'ai choisis le désert un peu par manque de compétence, puisque ce n'est pas un paysage très riche et un peu pour moi qui ai toujours voulu voyager dans l'Afrique du Nord et voir le désert. Ce que j'ai fini par faire lors de mon voyage en Egypte en 2023. Et c'étais aussi l'occasion de faire référence à mon pseudo, Arakin, que je tiens du nom de la capitale de la planète Arrakis, dans le film Dune (celui de 1984, que mon père m'avais montré 2 ans avant la sortie du magnifique film de Denis Villeneuve), que j'ai orthographié autrement. Et on retrouve bien sûr l'orange, ma couleur fétiche.",
		"format": "a4",
    },
	{
		"image": "205_gb",
		"nom": "2023 - Peugeot 205 Groupe B",
		"description": "Je suis complètement passionné d'automobile depuis plusieurs années. Et, certainement à cause de mon enfance puisque mes parents n'avaient que ça, de Peugeot. La Peugeot 205 T16 qui a couru en groupe B est une des voitures les plus incroyables qui aient existé, avec l'Audi Quattro S1 et la Lancia delta S4. J'ai donc pris beaucoup de plaisir à réaliser cette illustration sur le logiciel Adobe Illustrator.",
		"format": "square",
    },
	{
		"image": "206_wrc",
		"nom": "2023 - Peugeot 206 WRC",
		"description": "Après l'interdiction du championnat de rallye Groupe B est apparu le WRC. La Peugeot 206 WRC a gagné le championnat du monde des rallyes dès sa première inscription et trois années consécutives. Cette voiture a nettement participé à la promotion de sa petite sœur, la simple Peugeot 206. Une voiture que j'aime beaucoup aussi, que j'ai énormément conduit. Je me devais donc de rendre hommage à cette voiture dans sa version la plus incroyable : la WRC.",
		"format": "square",
    },
	{
		"image": "saab_93",
		"nom": "2022 - Saab 93 2.0l turbo",
		"description": "Cette voiture est celle de mon meilleur ami. Saab est une marque qui n'existe plus, et nous nous sommes rendu compte qu'il n'existait pas de merch pour cette marque alors nous avons créé le nôtre. J'ai illustré sa voiture et nous en avons fait des produits dérivés. C'était la première illustration de voiture que j'ai fait.",
		"format": "a4",
    },
	{
		"image": "logo_pp",
		"nom": "2021 - Mon logo",
		"description": "J’ai pendant longtemps conservé un simple A, pour Arakin, dans une police d’écriture gothique et sur fond noir. J’ai eu envie de quelque chose de plus moderne et personnalisé. J’ai alors commencé à travailler sur une forme proche du A plus moderne, ne conservant en fait que l’aspect triangulaire. Et c’est également ici que j’ai commencé à utiliser partout ma couleur préférée, l’orange.",
		"format": "square",
    },
	{
		"image": "aperobox",
		"nom": "2024 - Apéro Box",
		"description": "Dans le cadre de mes études, mon groupe et moi avons conçu un produit, une apérobox livrable tous les mois. Nous avons alors créé la WorldBox, une box apéro qui chaque mois vous fera découvrir les saveurs d'un nouveau pays. Je me suis occupé en grande partie du packaging. Mon travai était d'approté une identité coloré et premium au produit, c'est pourquoi j'ai choisis une belle boite à ouverture, sur laquelle j'ai designé un packaging élégant tout en respectant les contraintes du produit : une identité évoquatrice de la région du monde que couvre la box chaque mois. Voici, ici, la box Brésil.",
		"format": "square",
    },
	{
		"image": "maq_lazor",
		"nom": "2023 - Maquette pour Lazor-Thy",
		"description": "Au lycée, j’ai fait la rencontre de Reda, qui avait pour projet de lancer une marque de vêtement. J’ai travaillé pour lui afin de lui fournir des mockups (des maquettes 3d, comme ci-dessous) et j’ai également travaillé de mon côté sur cette piste, en récupérant les éléments qu’il m’a fournis pour ses vêtements et en les réagençant. Encore aujourd’hui, je travaille pour lui sur ce projet de marque de temps à autre.",
		"format": "square",
    },
	{
		"image": "logo_jlch",
		"nom": "2024 - Un logo pour mon père",
		"description": "Après sa galerie pour laquelle j’ai réalisé une affiche, mon père a ouvert une boutique avec une amie. «Les Jolies Choses», et il m’a demandé d’en réaliser le logo. Il m’avait demandé quelque chose de géométrique faisant apparaître les 3 mots du nom. Après de multiples propositions, celle-ci a été retenue. C’est à mon sens la plus esthétique, équilibrée et lisible. Et ce logo est orange, donc c’est mon préféré.",
		"format": "square",
    },
]

